...
export async function startPremBot(m, client, caption = '', isCode = false, phone = '', chatId = '', commandFlags = {}, isCommand = false) {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr }) => {
    ...
    if (connection === 'open') {
      ...
      console.log(chalk.yellow(`[ Sηαdοωβοτ ] PREM-BOT conectado: ${sock.userId}`))
    }
    ...
    if (connection === 'close') {
      ...
      setTimeout(() => startPremBot(m, client, caption, isCode, phone, chatId, {}, isCommand), 5000)
    }
    ...
    if (qr && isCode && phone && client && chatId && commandFlags[senderId]) {
      try {
        ...
        const msgCode = await client.sendMessage(chatId, { text: `*CÓDIGO PREM:* ${codeGen}` }, { quoted: msg });
        delete commandFlags[senderId];
      } catch (err) {
        console.error("[Prem-Code Error]", err);
      }
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (let raw of messages) {
      ...
      try {
        ...
        console.log(chalk.red(`[ Error Prem ] → ${err}`))
      }
    }
  })
  ...
  try {
    ...
    console.log(chalk.gray(`[ Prem Events ] → ${err}`))
  }
